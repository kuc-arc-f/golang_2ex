...
var db *sql.DB
var ADK_API_URL="http://localhost:8000"

type AdkRunReceiveItem struct {
  RcvText string `json:"text"`
}

type AdkPartItem struct {
    text string
}
type AdkMessageItem struct {
    role string
		parts []AdkPartItem
}

type AdkSendItem struct {
    appName string
    userId string
    sessionId string
		newMessage AdkMessageItem
}
